...
STOP_LOSS_PCT = 8       # -8%
HOLD_DAYS = "3-5天"     # 固定持仓时间

# 计算固定止盈止损价
TAKE_PROFIT = AVG_COST * (1 + TAKE_PROFIT_PCT / 100)  # 18.91
STOP_LOSS = AVG_COST * (1 - STOP_LOSS_PCT / 100)      # 15.13
...
def get_tech_indicators():
    ...
    try:
        ...
        print(f"技术指标更新: RSI={TECH_CACHE['rsi']}, MA10={TECH_CACHE['ma10']:.2f}")
...
def get_order_suggestions(price, tech):
    ...
    if price < TAKE_PROFIT:
        distance = (TAKE_PROFIT - price) / price * 100
        suggestions.append(f"📌 止盈挂单：{TAKE_PROFIT:.2f}元（距{distance:.1f}%）")
        suggestions.append(f"   └ 卖出{SHARES//2}股，回收{SHARES//2*TAKE_PROFIT:.0f}元")
    
    # 止损挂单（固定-8%）
    if price > STOP_LOSS:
        distance = (price - STOP_LOSS) / price * 100
        suggestions.append(f"📌 止损挂单：{STOP_LOSS:.2f}元（距{distance:.1f}%）")
        suggestions.append(f"   └ 清仓{SHARES}股，亏损{(STOP_LOSS-AVG_COST)*SHARES:.0f}元")
    
    # 技术指标参考
    suggestions.append(f"")
    suggestions.append(f"📊 技术参考（非执行依据）")
    suggestions.append(f"   └ MA10：{tech['ma10']:.2f}（跌破可提前减仓）")
    suggestions.append(f"   └ MA20：{tech['ma20']:.2f}（最后防线）")
    suggestions.append(f"   └ 近5日低点：{tech['support_5d']:.2f}")
...
def format_status(price_data, tag="", show_orders=False):
    ...
    msg += f"━━━━━━━━━━━━━━━━\n"
    msg += f"🎯 止盈：{TAKE_PROFIT:.2f} (+{TAKE_PROFIT_PCT}%)\n"
    msg += f"🚨 止损：{STOP_LOSS:.2f} (-{STOP_LOSS_PCT}%)\n"
    msg += f"⏰ 持仓：{HOLD_DAYS}\n"
    msg += f"━━━━━━━━━━━━━━━━\n"
    msg += f"📊 技术参考\n"
...
def main():
    ...
    # 接近止盈提醒
    if price >= TAKE_PROFIT * 0.97 and "warn_high" not in state["alerted_types"]:
        msg = f"📈 【接近止盈】\n"
        msg += f"{STOCK_NAME} 现价{price:.2f} (+{profit_pct:.1f}%)\n"
        msg += f"距止盈{TAKE_PROFIT:.2f}还差{(TAKE_PROFIT-price)/price*100:.1f}%\n\n"
    ...
    # 接近止损提醒
    if price <= STOP_LOSS * 1.03 and "warn_low" not in state["alerted_types"]:
        msg = f"📉 【接近止损】\n"
        msg += f"{STOCK_NAME} 现价{price:.2f} ({profit_pct:.1f}%)\n"
        msg += f"距止损{STOP_LOSS:.2f}还差{(price-STOP_LOSS)/price*100:.1f}%\n\n"
        msg += format_status(price_data, show_orders=True)
        send_wx(msg)
